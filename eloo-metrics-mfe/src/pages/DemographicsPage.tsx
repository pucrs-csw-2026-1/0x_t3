...
// US-03 — Distribuições demográficas. Exposta como remote (ADR-0005): recebe o
// tema por prop e não gerencia sessão/rota. Orquestra os 5 fetches independentes
// (faixa etária, gênero, cidade, perfil, tipo), o toolbar (evento + período) e a
// validação de intervalo ANTES do fetch (evita 422). Cada painel gerencia seu
// próprio estado de loading/erro/vazio; a falha de um não derruba os outros. Os
// gráficos recebem dados por props — nenhum faz fetch (ADR-0004).
export default function DemographicsPage({
  ...
  }, [validationError, period, eventId]);
  ...
  useEffect(() => {
    ...
    listEventMetrics({
      ...
      .then((result) => {
        if (!active) return;
        setEventOptions(
          result.items.map((item) => ({ id: item.eventId, name: item.eventName ?? item.eventId })),
        );
      })
      .catch(() => {
  ...
  }, [period.startDate, period.endDate, validationError]);
